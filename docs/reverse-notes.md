...
- 网关两根路径：
  ...
  - relay bridge：`POST {proxy}/api/electron/agent/send`（`{"args":[{sessionKey, message, model?, thinking?}]}`，注意 **`message` 必须是字符串**，对象形式返回 `message is required for agent.send`）+ `GET {proxy}/api/events`（SSE）。
